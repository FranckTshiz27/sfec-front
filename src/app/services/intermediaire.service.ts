...
export class IntermediaireService {
  ...
  getAll(): Observable<any> {
    return this.http.get<any>(
      `${environment.API.URL_BASE}${environment.API.SUB_RESSOURCE.INTERMEDIAIRE}`
    );
  }
  ...
  getAllForAdminUser(): Observable<any> {
    return this.http.get<any>(
      environment.API.URL_BASE +
        environment.API.SUB_RESSOURCE.INTERMEDIAIRE +
        environment.API.V1.GET_ALL_FOR_ADMIN_USER
    );
  }
  ...
  getAllSecondaire(): Observable<any> {
    return this.http.get<any>(
      environment.API.URL_BASE +
        environment.API.SUB_RESSOURCE.INTERMEDIAIRE +
        environment.API.V1.GET_ALL_SECONDARY
    );
  }
