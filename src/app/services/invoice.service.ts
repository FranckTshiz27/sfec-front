...
export class InvoiceService {
  ...
  pageSize: number = 0;
  pageIndex: number = 0;
  invoiceFilterText: string = '';
  selectedInvoice: any;
  isInvoiceSelected: boolean = true;

  // http request
  getAll(periodeDto: PeriodeDto): Observable<any> {
    return this.http.post<any>(
      `${environment.API.URL_BASE}${environment.API.SUB_RESSOURCE.INVOICE}`,
      periodeDto
    );
  }
  ...
  createDGIInvoice(invoice: Invoice) {
    return this.http.post(
      `${environment.API.URL_BASE}${environment.API.SUB_RESSOURCE.INVOICE}${environment.API.V1.CREATE_DGI_INVOICE}`,
      invoice
    );
  }

  searchInvoicesByReference(reference: string): Observable<Invoice[]> {
    return this.http.post<Invoice[]>(
      `${environment.API.URL_BASE}${environment.API.SUB_RESSOURCE.INVOICE}/search`,
      reference
    );
  }

  printerDGIInvoice(cle: string, amount: string): Observable<Blob> {
    return this.http
      .post<any>(
        `${environment.API.URL_BASE}${environment.API.SUB_RESSOURCE.INVOICE}${environment.API.V1.PRINT_DGI_INVOICE}/${cle}`,
        amount,
        {
